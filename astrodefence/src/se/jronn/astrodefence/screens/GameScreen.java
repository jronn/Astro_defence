...
public class GameScreen implements Screen{
	...
	public void render(float delta) {
		...
		for(int i = 0; i < 6; i++)
			for(int j = 0; j < 12; j++)
				batch.draw(space[i][j], j*Gdx.graphics.getHeight()/4 - Gdx.graphics.getHeight()/4, i*Gdx.graphics.getHeight()/4 - Gdx.graphics.getHeight()/8, Gdx.graphics.getHeight()/8, Gdx.graphics.getHeight()/8, Gdx.graphics.getHeight()/4, Gdx.graphics.getHeight()/4, 1, 1, (90*j)%360, false);
	...
	public void resize(int width, int height) {
		...
		if(prefs.getInteger("bomb") > 0 && prefs.getInteger("bomb") < 6) {
			go.getEarth().setBombRegen((0.5f + 1f * prefs.getInteger("bomb"))/2);
		}
		else {
