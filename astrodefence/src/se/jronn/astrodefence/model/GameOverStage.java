...
public class GameOverStage extends Stage{
	...
	public void setScore(int score) {
		...
		if(totalScore >= 7500) {
			...
			prefs.putInteger("upgradeToken", prefs.getInteger("upgradeToken") + (15 - prefs.getInteger("totalToken")));
			...
			int newTokens = (int)Math.floor((totalScore - (totalTokens * 500))/500);
			prefs.putInteger("totalToken", totalTokens + newTokens);
			prefs.putInteger("upgradeToken", prefs.getInteger("upgradeToken") + newTokens);
			
			if(newTokens > 0) {
				label_nextToken = new Label(newTokens + " NEW TOKEN EARNED!", new Label.LabelStyle(fontSmall, new Color(1, 1, 1, 1)));
				label_nextToken.setPosition(width/2 - label_nextToken.getWidth()/2, gameOverBg.getY() - label_nextToken.getHeight() * 1.5f);
			} else {
				label_nextToken = new Label("NEXT TOKEN " + (((totalTokens + 1) * 500) - totalScore) + " POINTS AWAY", new Label.LabelStyle(fontSmall, new Color(1, 1, 1, 1)));
				label_nextToken.setPosition(width/2 - label_nextToken.getWidth()/2, gameOverBg.getY() - label_nextToken.getHeight() * 1.5f);
			}
		} 
